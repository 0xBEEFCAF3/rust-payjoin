...
impl Into<Vec<u8>> for UrlWrapper {
    fn into(self) -> Vec<u8> { self.0.to_string().into_bytes() }
}

impl TryFrom<Vec<u8>> for UrlWrapper {
...
impl NewSender {
    ...
    pub fn persist<P: Persister<Sender>>(
        ...
        let url = UrlWrapper(self.v1.endpoint.clone());
        let sender = Sender { v1: self.v1.clone(), reply_key: self.reply_key.clone() };
        let token = persister.save(url, sender)?;
        Ok(token)
    }
}
