...
impl SenderTypeState {
    fn process_event(self, event: SessionEvent) -> Result<SenderTypeState, ReplayError> {
        match (self, event) {
            ...
            (SenderTypeState::V2GetContext(state), SessionEvent::ProposalReceived(proposal)) =>
...
impl Sender<V2GetContext> {
    ...
    > {
        let body = match process_get_res(response, ohttp_ctx) {
            Ok(Some(body)) => body,
            Ok(None) => return MaybeFatalTransitionWithNoResults::no_results(self.clone()),
            Err(e) =>
        ...
        MaybeFatalTransitionWithNoResults::success(
            SessionEvent::ProposalReceived(processed_proposal.clone()),
            Sender { state: ProposalReceived(processed_proposal) },
        )
    }

    pub(crate) fn apply_proposal_received(self, proposal: Psbt) -> SenderTypeState {
        SenderTypeState::ProposalReceived(Sender { state: ProposalReceived(proposal) })
    }
...
#[derive(Debug, Clone)]
pub struct ProposalReceived(pub(crate) Psbt);
impl SenderState for ProposalReceived {}
