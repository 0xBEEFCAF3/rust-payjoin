...
impl SessionHistory {
    pub fn proposed_psbt(&self) -> Option<&Psbt> {
        self.events.iter().find_map(|event| match event {
            SessionEvent::ProposalReceived(psbt) => Some(psbt),
            _ => None,
        })
    }
    ...
    fn sender_with_reply_key(&self) -> Option<&WithReplyKey> {
        self.events.iter().find_map(|event| match event {
            SessionEvent::CreatedReplyKey(sender_with_reply_key) => Some(sender_with_reply_key),
            _ => None,
        })
    }
    ...
    pub fn session_invalid(&self) -> Option<String> {
        self.events.iter().find_map(|event| match event {
            SessionEvent::SessionInvalid(e) => Some(e.clone()),
            _ => None,
        })
    }
}
