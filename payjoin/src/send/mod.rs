...
fn find_change_index(
    ...
    Ok(Some(AdditionalFeeContribution {
        amount: check_fee_output_amount(output, fee, clamp_fee_contribution)?,
        vout: index,
    }))
}
...
fn serialize_url(
    ...
    if let Some(AdditionalFeeContribution { amount, vout }) = fee_contribution {
        url.query_pairs_mut()
    ...
    pub(crate) fn create_psbt_context() -> super::PsbtContext {
        ...
        super::PsbtContext {
            ...
            fee_contribution: Some(AdditionalFeeContribution {
                amount: bitcoin::Amount::from_sat(182),
                vout: 0,
            }),
