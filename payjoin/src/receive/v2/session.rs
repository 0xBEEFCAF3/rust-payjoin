...
/// `SessionHistory` is a collection of events that have occurred during a receiver's session.
/// It is obtained by calling [replay_receiver_event_log].
...
mod tests {
    ...
    fn run_session_history_test(test: SessionHistoryTest) {
        ...
        assert_eq!(session_history.pj_uri_amount(), test.expected_session_history.payment_amount);
