        ...
        for (key, v) in pairs {
            match (key.borrow(), v.borrow()) {
                ...
                ("disableoutputsubstitution", v) =>
                    params.disable_output_substitution = v == "1",
                _ => (),
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_params_from_query_pairs() {
        // Urls are generated by send/mod.rs::serialize_url
        let url = Url::parse("http://localhost?v=2&disableoutputsubstitution=1&maxadditionalfeecontribution=100000000&additionalfeeoutputindex=0&minfeerate=1000").unwrap();
        let params = Params::from_query_pairs(url.query_pairs(), &[2]).unwrap();
        assert_eq!(params.v, 2);
        assert_eq!(params.disable_output_substitution, true);
        assert_eq!(params.additional_fee_contribution, Some((bitcoin::Amount::from_sat(100000000), 0)));
        assert_eq!(params.min_feerate, FeeRate::from_sat_per_vb(1000).expect("valid feerate"));
    }
}
