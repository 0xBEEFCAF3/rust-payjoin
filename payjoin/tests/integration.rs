...
mod integration {
    ...
    mod v2 {
        ...
        use payjoin_test_utils::{init_bitcoind_multi_sender_single_reciever, TestServices};
        ...
        async fn test_ns1r() {
            ...
            err = services.take_ohttp_relay_handle().unwrap() => panic!("Ohttp relay exited early: {:?}", err),
            err = services.take_directory_handle().unwrap() => panic!("Directory server exited early: {:?}", err),
            res = test_ns1r(&services) => assert!(res.is_ok(), "v2 send receive failed: {:#?}", res)
            );
            ...
            async fn test_ns1r(services: &TestServices) -> Result<(), BoxError> {
                ...
                let mut multi_party_proposal =
                    payjoin::multi_party::UncheckedProposalBuilder::new();
                ...
                for (i, sender_sesssion) in inner_sender_test_sessions.iter().enumerate() {
                    ...
                    let checked_payjoin_proposal_psbt = sender_ctx
                        .process_response(response.bytes().await?.to_vec().as_slice(), ohttp_ctx)?
                    ...
                    let response = agent
                        ...
                        .unwrap();
                    assert!(response.status().is_success());
                    send_post_ctx.process_response(response.bytes().await?.to_vec().as_slice())?;
                }

                //**********************
                // Inside the Receiver:
                // Reciever should pull the final psbts from all sub dirs
                let mut finalized_proposals = payjoin::multi_party::FinalizedProposal::new();
                for sender_sesssion in inner_sender_test_sessions.iter() {
                    let mut receiver_session = sender_sesssion.receiver_session.clone();
                    let (req, reciever_ctx) = receiver_session.extract_req(&directory)?;
                    let response = agent.post(req.url).body(req.body).send().await?;
                    ...
                    let finalized_response = receiver_session
                        ...
                        .unwrap();
                    finalized_proposals.add(finalized_response).expect("valid finalized proposal");
                }

                let agg_psbt = finalized_proposals.combine().expect("valid finalized proposal");
                // Check resulting transaction and balances
        ...
        fn handle_multi_party_proposal(
            receiver: &bitcoincore_rpc::Client,
            multi_party_proposal: payjoin::multi_party::UncheckedProposal,
            // TODO (armins): add custom inputs function param
        ) -> payjoin::multi_party::PayjoinProposal {
            ...
            let selected_inputs = {
                ...
                let selected_input = candidate_inputs.next().unwrap();
                vec![selected_input]
            };
