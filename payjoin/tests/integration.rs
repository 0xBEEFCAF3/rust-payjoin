    ...
    mod v2 {
        ...
        async fn test_bad_ohttp_keys() -> Result<(), BoxSendSyncError> {
            ...
            async fn try_request_with_bad_keys(
                ...
                let mut bad_initializer = Receiver::load(&storage_token, &persister)?;
        ...
        async fn test_session_expiration() -> Result<(), BoxSendSyncError> {
            ...
            async fn do_expiration_tests(services: &TestServices) -> Result<(), BoxError> {
                ...
                let storage_token = new_receiver.persist(&mut persister)?;
                let mut expired_receiver = Receiver::load(&storage_token, &persister)?;
                ...
                let expired_req_ctx = Sender::load(&storage_token, &mut sender_persister)?;
        ...
        async fn v2_to_v2() -> Result<(), BoxSendSyncError> {
            ...
            async fn do_v2_send_receive(services: &TestServices) -> Result<(), BoxError> {
                ...
                let storage_token = new_receiver.persist(&mut persister)?;
                let mut session = Receiver::load(&storage_token, &persister)?;
        ...
        async fn ns1r() -> Result<(), BoxSendSyncError> {
            ...
            async fn do_ns1r(services: &TestServices) -> Result<(), BoxError> {
                ...
                for sender in senders.iter() {
                    ...
                    let storage_token = new_receiver.persist(&mut persister)?;
                    let receiver_session = Receiver::load(&storage_token, &persister)?;
                    let pj_uri = receiver_session.pj_uri();
                    let psbt = build_sweep_psbt(sender, &pj_uri)?;
                    let mut sender_persister = NoopPersister;
                    let sender_ctx = MultiPartySenderBuilder::new(psbt.clone(), pj_uri.clone())
                        .build_recommended(FeeRate::BROADCAST_MIN)?;
                    let storage_token = sender_ctx.persist(&mut sender_persister)?;
                    let req_ctx = Sender::load(&storage_token, &sender_persister)?;
