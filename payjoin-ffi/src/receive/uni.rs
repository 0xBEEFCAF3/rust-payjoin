...
pub struct ReceiverSessionEvent(super::ReceiverSessionEvent);

impl From<ReceiverSessionEvent> for super::ReceiverSessionEvent {
    fn from(value: ReceiverSessionEvent) -> Self { value.0 }
}

impl From<super::ReceiverSessionEvent> for ReceiverSessionEvent {
    fn from(value: super::ReceiverSessionEvent) -> Self { ReceiverSessionEvent(value) }
}
...
impl ReceiverSessionEvent {
    ...
    pub fn from_json(json: String) -> Result<Self, SerdeJsonError> {
        let event: payjoin::receive::v2::ReceiverSessionEvent = serde_json::from_str(&json)?;
...
/// Adapter for the JsonReceiverPersistedSession trait to use the save and load callbacks.
