...
impl From<payjoin::receive::v2::ReceiverSessionEvent> for ReceiverSessionEvent {
    fn from(event: payjoin::receive::v2::ReceiverSessionEvent) -> Self { Self(event) }
}
...
pub struct ReceiverState(pub payjoin::receive::v2::ReceiverTypeState);

impl From<payjoin::receive::v2::ReceiverTypeState> for ReceiverState {
...
where
    P: SessionPersister,
    P::SessionEvent: Into<payjoin::receive::v2::ReceiverSessionEvent> + Clone,
{
    let (state, history) = payjoin::receive::v2::replay_receiver_event_log(persister)
        .map_err(ReceiverReplayError::from)?;
