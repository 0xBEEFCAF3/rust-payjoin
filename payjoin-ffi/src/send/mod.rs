...
use crate::send::error::ReplayError;
use crate::uri::PjUri;
...
#[derive(Debug, Clone)]
pub struct SenderState(payjoin::send::v2::SenderTypeState);

impl From<payjoin::send::v2::SenderTypeState> for SenderState {
    fn from(value: payjoin::send::v2::SenderTypeState) -> Self { Self(value) }
}

pub fn replay_event_log<P>(persister: &P) -> Result<(SenderState, SessionHistory), ReplayError>
where
    P: SessionPersister + Clone,
    P::SessionEvent: Into<payjoin::send::v2::SessionEvent> + Clone,
{
    let (state, history) =
        payjoin::send::v2::replay_event_log(persister).map_err(ReplayError::from)?;
...
pub struct V2GetContextTransitionOutcome(
    payjoin::persist::OptionalTransitionOutcome<
        payjoin::send::v2::Sender<payjoin::send::v2::ProposalReceived>,
        payjoin::send::v2::Sender<payjoin::send::v2::V2GetContext>,
    >,
);
...
impl V2GetContextTransitionOutcome {
    ...
    pub fn success(&self) -> Option<ProposalReceived> {
...
impl V2GetContext {
    ...
    pub fn process_response(
        ...
        V2GetContextTransition(Arc::new(RwLock::new(Some(
            self.clone().0.process_response(response, ohttp_ctx.into()),
        ))))
    }
}

pub struct ProposalReceived(payjoin::send::v2::Sender<payjoin::send::v2::ProposalReceived>);

impl From<payjoin::send::v2::Sender<payjoin::send::v2::ProposalReceived>> for ProposalReceived {
    fn from(value: payjoin::send::v2::Sender<payjoin::send::v2::ProposalReceived>) -> Self {
        Self(value)
    }
}
