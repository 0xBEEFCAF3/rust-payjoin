...
pub enum SenderState {
    ...
    ProposalReceived { inner: Arc<ProposalReceived> },
    TerminalState,
}

impl From<super::SenderState> for SenderState {
    fn from(value: super::SenderState) -> Self {
        ...
        match value.0 {
            ...
            ProposalReceived(inner) => Self::ProposalReceived {
                inner: Arc::new(super::ProposalReceived::from(inner).into()),
            },
...
impl SenderReplayResult {
    pub fn state(&self) -> SenderState { self.state.clone() }
...
) -> Result<SenderReplayResult, ReplayError> {
...
impl V2GetContextTransitionOutcome {
    ...
    pub fn success(&self) -> Option<Arc<ProposalReceived>> {
        self.0.success().map(|p| Arc::new(ProposalReceived(p.into())))
    }
}
...
#[derive(uniffi::Object)]
pub struct ProposalReceived(super::ProposalReceived);

impl From<super::ProposalReceived> for ProposalReceived {
    fn from(value: super::ProposalReceived) -> Self { Self(value) }
}

#[uniffi::export]
impl ProposalReceived {
    pub fn psbt(&self) -> Psbt { self.0.psbt() }
}
