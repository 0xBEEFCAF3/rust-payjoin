    ...
    async fn v2_to_v2_full_cycle() {
        ...
        async fn do_v2_send_receive(services: &TestServices) -> Result<(), BoxError> {
            ...
            let response = agent
                ...
                .unwrap();
            assert!(response.status().is_success());
            let send_ctx = req_ctx.process_response(&response.bytes().await?, context).save(&sender_session_persister)?;
            ...
            let checked_payjoin_proposal_psbt =
                send_ctx.process_response(&response.bytes().await?, &ohttp_ctx).save(&sender_session_persister)?.success().unwrap().psbt();
            let payjoin_tx = extract_pj_tx(&sender, checked_payjoin_proposal_psbt.serialize_base64().as_str())?;
